...
func (obj RemoveBalanceLiquidity) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	...
	if err = encoder.Encode(obj.MinimumATokenOut); err != nil {
		return fmt.Errorf("error while marshaling minimumATokenOutParam:%w", err)
	}
	// Serialize `minimumBTokenOutParam`:
	if err = encoder.Encode(obj.MinimumBTokenOut); err != nil {
		return fmt.Errorf("error while marshaling minimumBTokenOutParam:%w", err)
	}
	return nil
}

func (obj *RemoveBalanceLiquidity) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	...
	if err = decoder.Decode(&obj.MinimumATokenOut); err != nil {
		return fmt.Errorf("error while unmarshaling MinimumATokenOut:%w", err)
	}
	// Deserialize `MinimumBTokenOut`:
	if err = decoder.Decode(&obj.MinimumBTokenOut); err != nil {
		return fmt.Errorf("error while unmarshaling MinimumBTokenOut:%w", err)
	}
	return nil
}

func (obj *RemoveBalanceLiquidity) SetAccounts(accounts solanago.PublicKeySlice) (err error) {
	if len(accounts) < 16 {
...
func (obj *RemoveBalanceLiquidity) PublicKeys() solanago.PublicKeySlice {
	return obj.PublicKeySlice
}

func (*RemoveBalanceLiquidity) TypeID() binary.TypeID {
	return binary.TypeIDFromBytes(Instruction_RemoveBalanceLiquidity)
}

func (*RemoveBalanceLiquidity) NewInstance() programparser.Instruction {
...
func NewRemoveBalanceLiquidityInstruction(
	...
	if err = enc__.WriteBytes(Instruction_RemoveBalanceLiquidity[:], false); err != nil {
...

	{
		...
		// Serialize `minimumATokenOutParam`:
		if err = enc__.Encode(minimumATokenOutParam); err != nil {
			return nil, fmt.Errorf("error while marshaling minimumATokenOutParam:%w", err)
		}
		// Serialize `minimumBTokenOutParam`:
		if err = enc__.Encode(minimumBTokenOutParam); err != nil {
			return nil, fmt.Errorf("error while marshaling minimumBTokenOutParam:%w", err)
		}
	}
...
// Builds a "BuildRemoveBalanceLiquidity" instruction.
// Withdraw tokens from the pool in a balanced ratio. User will still able to withdraw from pool even the pool is disabled. This allow user to exit their liquidity when there's some unforeseen event happen.
func BuildRemoveBalanceLiquidity(
	// Params:
	poolTokenAmountParam uint64,
	minimumATokenOutParam uint64,
	minimumBTokenOutParam uint64,
	...
	instruction_, _ := NewRemoveBalanceLiquidityInstruction(
		poolTokenAmountParam,
		minimumATokenOutParam,
		minimumBTokenOutParam,
		pool,
		lpMint,
		userPoolLp,
		aVaultLp,
