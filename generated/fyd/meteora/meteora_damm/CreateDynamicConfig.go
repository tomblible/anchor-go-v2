// Code generated by https://github.com/gagliardetto/anchor-go. DO NOT EDIT.

package meteora_damm

import (
	"bytes"
	"fmt"

	binary "github.com/gagliardetto/binary"
	solanago "github.com/gagliardetto/solana-go"
	programparser "github.com/yydsqu/solana-sdk/program_parser"
)

// Builds a "create_dynamic_config" instruction.
type CreateDynamicConfig struct {
	// Params:
	Index            uint64
	ConfigParameters DynamicConfigParameters
	// Accounts:
	// [0] = [writable] config
	Config solanago.PublicKey `bin:"-"`
	// [1] = [writable,signer] admin
	Admin solanago.PublicKey `bin:"-"`
	// [2] = [] system_program
	SystemProgram solanago.PublicKey `bin:"-"`
	// [3] = [] event_authority
	EventAuthority solanago.PublicKey `bin:"-"`
	// [4] = [] program
	Program solanago.PublicKey `bin:"-"`
	// PublicKeySlice
	solanago.PublicKeySlice `bin:"-"`
}

func (obj CreateDynamicConfig) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `indexParam`:
	if err = encoder.Encode(obj.Index); err != nil {
		return fmt.Errorf("error while marshaling indexParam:%w", err)
	}
	// Serialize `configParametersParam`:
	if err = encoder.Encode(obj.ConfigParameters); err != nil {
		return fmt.Errorf("error while marshaling configParametersParam:%w", err)
	}
	return nil
}

func (obj *CreateDynamicConfig) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `Index`:
	if err = decoder.Decode(&obj.Index); err != nil {
		return fmt.Errorf("error while unmarshaling Index:%w", err)
	}
	// Deserialize `ConfigParameters`:
	if err = decoder.Decode(&obj.ConfigParameters); err != nil {
		return fmt.Errorf("error while unmarshaling ConfigParameters:%w", err)
	}
	return nil
}

func (obj *CreateDynamicConfig) SetAccounts(accounts solanago.PublicKeySlice) (err error) {
	if len(accounts) < 5 {
		return fmt.Errorf("too few accounts, expect %d actual %d", 5, len(accounts))
	}
	obj.Config = accounts[0]
	obj.Admin = accounts[1]
	obj.SystemProgram = accounts[2]
	obj.EventAuthority = accounts[3]
	obj.Program = accounts[4]
	obj.PublicKeySlice = accounts
	return nil
}
func (obj *CreateDynamicConfig) PublicKeys() solanago.PublicKeySlice {
	return obj.PublicKeySlice
}

func (*CreateDynamicConfig) TypeID() binary.TypeID {
	return binary.TypeIDFromBytes(Instruction_CreateDynamicConfig)
}

func (*CreateDynamicConfig) NewInstance() programparser.Instruction {
	return new(CreateDynamicConfig)
}

func (obj *CreateDynamicConfig) GetRemainingAccounts() solanago.PublicKeySlice {
	if len(obj.PublicKeySlice) <= 5 {
		return nil
	}
	return obj.PublicKeySlice[5:]
}

// Builds a "create_dynamic_config" instruction.
func NewCreateDynamicConfigInstruction(
	// Params:
	indexParam uint64,
	configParametersParam DynamicConfigParameters,

	// Accounts:
	config solanago.PublicKey,
	admin solanago.PublicKey,
	eventAuthority solanago.PublicKey,
	program solanago.PublicKey,
	remaining__ ...*solanago.AccountMeta,
) (*solanago.GenericInstruction, error) {
	var (
		err    error
		buf__  = new(bytes.Buffer)
		enc__  = binary.NewBorshEncoder(buf__)
		metas_ = make(solanago.AccountMetaSlice, 5, 5+len(remaining__))
	)

	// Encode the instruction discriminator.
	if err = enc__.WriteBytes(Instruction_CreateDynamicConfig[:], false); err != nil {
		return nil, fmt.Errorf("failed to write instruction discriminator: %w", err)
	}

	{
		// Serialize `indexParam`:
		if err = enc__.Encode(indexParam); err != nil {
			return nil, fmt.Errorf("error while marshaling indexParam:%w", err)
		}
		// Serialize `configParametersParam`:
		if err = enc__.Encode(configParametersParam); err != nil {
			return nil, fmt.Errorf("error while marshaling configParametersParam:%w", err)
		}
	}

	// Add the accounts to the instruction.
	{
		// [0] = [writable] config
		metas_[0] = solanago.NewAccountMeta(config, true, false)
		// [1] = [writable,signer] admin
		metas_[1] = solanago.NewAccountMeta(admin, true, true)
		// [2] = [] system_program
		metas_[2] = solanago.NewAccountMeta(SystemProgram, false, false)
		// [3] = [] event_authority
		metas_[3] = solanago.NewAccountMeta(eventAuthority, false, false)
		// [4] = [] program
		metas_[4] = solanago.NewAccountMeta(program, false, false)
		// append remaining metas
		metas_ = append(metas_, remaining__...)
	}

	// Create the instruction.
	return solanago.NewInstruction(
		ProgramID,
		metas_,
		buf__.Bytes(),
	), nil
}

// Builds a "BuildCreateDynamicConfig" instruction.
func BuildCreateDynamicConfig(
	// Params:
	indexParam uint64,
	configParametersParam DynamicConfigParameters,

	// Accounts:
	config solanago.PublicKey,
	admin solanago.PublicKey,
	eventAuthority solanago.PublicKey,
	program solanago.PublicKey,
	remaining__ ...*solanago.AccountMeta,
) *solanago.GenericInstruction {
	instruction_, _ := NewCreateDynamicConfigInstruction(
		indexParam,
		configParametersParam,
		config,
		admin,
		eventAuthority,
		program,
		remaining__...,
	)
	return instruction_
}
