...
func (g *Generator) genfile_types() ([]*OutputFile, error) {
	var (
		...
		accountFile = &OutputFile{
			Name: "accounts.go",
			File: NewFile(g.options.Package),
		}
		eventFile = &OutputFile{
			Name: "events.go",
			File: NewFile(g.options.Package),
		}
	)
	typeFile.File.HeaderComment("Code generated by https://github.com/gagliardetto/anchor-go. DO NOT EDIT.")
	typeFile.File.HeaderComment("This file contains parsers for the types defined in the IDL.")
	// 添加默认配置
	{
		typeFile.File.Add(Empty().
			Type().Id("Instruction").
			InterfaceFunc(func(group *Group) {
				group.Id("TypeID").Params().Qual(PkgBinary, "TypeID")
				group.Id("SetAccounts").
					Params(Id("accounts").Qual(PkgSolanaGo, "PublicKeySlice")).
					Params(Error())
				group.Id("Copy").Params().Id("Instruction")
			}).Line(),
		)
	}

	{
		for index, typ := range g.idl.Types {
			switch {
			case g.isAccounts(typ.Name):
				withDiscriminator := true
				for _, account := range g.idl.Accounts {
					if account.Name == typ.Name {
						if len(account.Discriminator) == 0 {
							withDiscriminator = false
						}
						break
					}
				}
				code, err := g.gen_IDLTypeDef(typ, withDiscriminator)
				if err != nil {
					return nil, fmt.Errorf("error generating type %d: %w", index, err)
				}
				accountFile.File.Add(code)
			case g.isEvents(typ.Name):
				withDiscriminator := true
				for _, account := range g.idl.Events {
					if account.Name == typ.Name {
						if len(account.Discriminator) == 0 {
							withDiscriminator = false
						}
				...
				eventFile.File.Add(code)
			default:
				code, err := g.gen_IDLTypeDef(typ, false)
				if err != nil {
					return nil, fmt.Errorf("error generating type %d: %w", index, err)
				}
				typeFile.File.Add(code)
			}
		}
	}

	return []*OutputFile{typeFile, accountFile, eventFile}, nil
}

func (g *Generator) isAccounts(name string) bool {
	for _, account := range g.idl.Accounts {
...
func (g *Generator) isEvents(name string) bool {
	for _, account := range g.idl.Events {
		if account.Name == name {
			return true
		}
...
func (g *Generator) gen_IDLTypeDef(def idl.IdlTypeDef, withDiscriminator bool) (Code, error) {
	switch vv := def.Ty.(type) {
	case *idl.IdlTypeDefTyStruct:
		return g.genIdlTypeDeftyStruct(def.Name, def.Docs, *vv, withDiscriminator)
