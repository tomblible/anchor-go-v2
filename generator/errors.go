...
func (g *Generator) gen_errors() (*OutputFile, error) {
	...
	file.HeaderComment("This file contains errors.")
	{
		...
		// 创建全局ErrorCode
		errorCodes := Empty()
		errorCodes.Comment("error codes")
		errorCodes.Line()
		errorCodes.Add(Empty().Type().Id("ErrorCode").Id("uint32"))
		errorCodes.Line()
		errorCodes.Line()

		errorCodes.Const().Parens(
			DoGroup(
				func(code *Group) {
					code.Line()
					for _, err := range g.idl.Errors {
						if err.Msg.IsSome() {
							code.Comment(err.Msg.Unwrap())
						} else {
							code.Comment(err.Name)
						}
						code.Line()
						code.Id("Err" + err.Name).Op("ErrorCode =").Lit(int(err.Code))
						code.Line()
					}
				},
			),
